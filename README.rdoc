...
This is s starter project for a framework called Dubious

==Setup the Duby development environment 

Fetch App Engine Java SDK 1.3.5, and install it in /usr/local

http://googleappengine.googlecode.com/files/appengine-java-sdk-1.3.5.zip

Fetch JRuby 1.5.1, and install is as well
...
Edit .bash_profile to find these

   export JRUBY_HOME=/usr/local/jruby-1.5.1
   export DUBY_HOME=/Users/username/mystuff/duby
...
Now build and run the app locally

   ./script/build.sh
   ./script/server.sh

Then publish to appspot.com
